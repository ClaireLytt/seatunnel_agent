...
contents, metrics, counts, or any question that implies reading from Hive
tables. Typical signals: table names, Chinese business terms matching schema
comments, time range references, aggregation words (总量/count/sum).
...
- Only tables in `config/schema_ddl.sql` can be queried (whitelist).
- Partitioned tables must have a partition filter — rejected otherwise.
